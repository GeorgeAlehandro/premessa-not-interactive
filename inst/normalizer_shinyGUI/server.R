                   ...
                   fluidRow(
                       ...
                       column(6,
                              conditionalPanel(
                                  ...
                                  fileInput("beads_file", "Choose files", multiple = TRUE),
                                  actionButton("submit_beads", "Submit beads folder"),
    ...
    observeEvent(input$submit_analysis, {
        ...
        output$normalizerui_dialog1 <- renderText({
            if(!is.null(input$normalizerui_baseline) && length(grep("^Existing", input$normalizerui_baseline)) > 0) {
                files = fixUploadedFilesNames(input$beads_file)
                files <<- files
                print(files)
                normalizerui.baseline.dir <<- dirname(files$datapath[1])
